...
public class MainRegistry {
    ...
    public MainRegistry(FMLJavaModLoadingContext context) {
        ...
        if (!ModFeatureFlags.DOORS_ONLY) {
            ...
            LOGGER.info("DOORS_ONLY mode: radiation and player rad handlers are not registered.");
        }
    ...
    private void commonSetup(final FMLCommonSetupEvent event) {
        event.enqueueWork(() -> {
            ...
            ModHazards.registerHazards(); // no-op when DOORS_ONLY
    ...
    private void addCreative(BuildCreativeModeTabContentsEvent event) {
        ...
        if (event.getTab() == ModCreativeTabs.NTM_MACHINES_TAB.get() || event.getTabKey() == CreativeModeTabs.SEARCH) {
            ...
            event.accept(ModBlocks.CRATE_TEMPLATE);
...
// 2. Проходимся по списку и добавляем 2 версии каждой
    ...
    /**
     * Creative inventory for the doors-only build: one tab plus search, no other HBM content.
     */
    private static void addDoorsOnlyCreative(BuildCreativeModeTabContentsEvent event) {
        boolean fill = event.getTab() == ModCreativeTabs.NTM_BUILDING_TAB.get()
        ...
        event.accept(ModBlocks.DOOR_BUNKER);
        event.accept(ModBlocks.METAL_DOOR);
        event.accept(ModItems.LARGE_VEHICLE_DOOR);
        event.accept(ModItems.ROUND_AIRLOCK_DOOR);
        event.accept(ModItems.TRANSITION_SEAL);
        event.accept(ModItems.SILO_HATCH);
        event.accept(ModItems.SILO_HATCH_LARGE);
        event.accept(ModItems.QE_CONTAINMENT);
        event.accept(ModItems.WATER_DOOR);
        event.accept(ModItems.FIRE_DOOR);
        event.accept(ModItems.SLIDE_DOOR);
        event.accept(ModItems.SLIDING_SEAL_DOOR);
        event.accept(ModItems.SECURE_ACCESS_DOOR);
        event.accept(ModItems.QE_SLIDING);
        event.accept(ModItems.VAULT_DOOR);
    }
