...
public class ChunkRadiationManager {
    ...
    public static float getRadiation(Level level, int x, int y, int z) {
        if (ModFeatureFlags.DOORS_ONLY || !ModClothConfig.get().enableRadiation || !ModClothConfig.get().enableChunkRads) return 0F;
        return getProxy().getRadiation(level, x, y, z);
    }

    public static void setRadiation(Level level, int x, int y, int z, float rad) {
        if (ModFeatureFlags.DOORS_ONLY || !ModClothConfig.get().enableRadiation || !ModClothConfig.get().enableChunkRads) return;
